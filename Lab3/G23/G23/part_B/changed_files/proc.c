...
struct request_queue rqueue_out;

struct proc *dequeue_out()
{

  acquire(&rqueue_out.lock);
  if (rqueue_out.s == rqueue_out.e)
  {
    release(&rqueue_out.lock);
    return 0;
  }
  struct proc *p = rqueue_out.queue[rqueue_out.s];
  (rqueue_out.s)++;
  (rqueue_out.s) %= NPROC;
  release(&rqueue_out.lock);
...
int enqueue_out(struct proc *p)
{

  acquire(&rqueue_out.lock);
  if ((rqueue_out.e + 1) % NPROC == rqueue_out.s)
  {
    release(&rqueue_out.lock);
    return 0;
  }
  rqueue_out.queue[rqueue_out.e] = p;
  rqueue_out.e++;
  (rqueue_out.e) %= NPROC;
  release(&rqueue_out.lock);
...
struct request_queue rqueue_in;

struct proc *dequeue_in()
{

  acquire(&rqueue_in.lock);
  if (rqueue_in.s == rqueue_in.e)
  {
    release(&rqueue_in.lock);
    return 0;
  }
  struct proc *p = rqueue_in.queue[rqueue_in.s];
  (rqueue_in.s)++;
  (rqueue_in.s) %= NPROC;
  release(&rqueue_in.lock);
  return p;
...
int enqueue_in(struct proc *p)
{
  acquire(&rqueue_in.lock);
  if ((rqueue_in.e + 1) % NPROC == rqueue_in.s)
  {
    release(&rqueue_in.lock);
    return 0;
  }
  rqueue_in.queue[rqueue_in.e] = p;
  (rqueue_in.e)++;
  (rqueue_in.e) %= NPROC;
...
void pinit(void)
{
  ...
  initlock(&rqueue_out.lock, "rqueue_out");
  initlock(&sleeping_channel_lock, "sleeping_channel");
  initlock(&rqueue_in.lock, "rqueue_in");
}
